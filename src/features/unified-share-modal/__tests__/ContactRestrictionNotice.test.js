...
describe('features/unified-share-modal/ContactRestrictionNotice', () => {
    ...
    const getWrapper = (props = {}) => {
        return shallow(
            <ContactRestrictionNotice
                ...
                isRestrictionJustificationAllowed={false}
    ...
    describe('render', () => {
        ...
        test.each`
            isRestrictionJustificationAllowed | restrictionNoticeMessageId                         | removeButtonMessageId
        ...
        `(
            'should select appropriate messages when isRestrictionJustificationAllowed is $isRestrictionJustificationAllowed',
            ({ isRestrictionJustificationAllowed, restrictionNoticeMessageId, removeButtonMessageId }) => {
                wrapper.setProps({ isRestrictionJustificationAllowed });
        ...
        test('should render a loading indicator instead of the justification reasons select when isFetchingJustificationReasons is true', () => {
            wrapper.setProps({ isFetchingJustificationReasons: true, isRestrictionJustificationAllowed: true });

            expect(wrapper.find('[data-resin-target="justificationReasonsSelect"]')).toHaveLength(0);
            expect(wrapper.find('LoadingIndicator')).toHaveLength(1);
        });

        test('should render justification reasons select when isFetchingJustificationReasons is false and isRestrictionJustificationAllowed is true', () => {
            ...
            wrapper.setProps({
                isFetchingJustificationReasons: false,
                isRestrictionJustificationAllowed: true,
                justificationReasons,
                selectedJustificationReason,
            });
    ...
    describe('handlers', () => {
        ...
        test('should call onSelectJustificationReason with newly selected option on justification reason select change', () => {
            ...
            wrapper.setProps({ isRestrictionJustificationAllowed: true, onSelectJustificationReason });
