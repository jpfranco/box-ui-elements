...
class UnifiedShareForm extends React.Component<USFProps, State> {
    ...
    handleSendInvites = (data: Object) => {
        ...
        if (!!submitJustificationReason && shouldSubmitJustificationReason) {
            // Confirm restriction status by calling invites api prior to submitting
            // justification. This will cover cases such as the user adding additional
            // restricted emails, restriction being removed or changed, etc.
            return sendInvites(params).catch(error => {
                const { restrictedEmails } = error;
                // Submit justification with updated restricted email list. The
                // invites api call will succeed once there exists a justification
                // for the restricted emails
                return submitJustificationReason({
                    checkpoint: JUSTIFICATION_CHECKPOINT_EXTERNAL_COLLAB,
                    classificationLabelId,
                    emails: restrictedEmails,
                    itemTypedID: item.typedID,
                    justificationReason: {
